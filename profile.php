...
if ( ! empty($_GET) ) {		// изменение страницы отображения списка пользователей
	if ( isset($_GET['del']) ) $_SESSION['settings']->message = del_user( $_GET['del'] ); 		// удаление
	if ( isset($_GET['st']) ) $_SESSION['settings']->message = state_user( $_GET['st'] ); 		// состояние
